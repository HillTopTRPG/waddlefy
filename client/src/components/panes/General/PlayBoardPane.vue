        ...
        <v-card>
          <v-card-text>
            <v-color-picker
              ...
              :swatches="
                store.mapMasks.value
                  .map((mm: { bg_color: string }) => mm.bg_color)
        ...
        >
          ...
          <v-list-item>
            <v-text-field
              label="名前"
              type="text"
              v-model="store.playBoards.value.find((pb: { uuid: string }) => pb.uuid === currentBoardUuid).name"
          ...
          <v-list-item>
            <v-select
              label="グリッドタイプ"
              v-model="store.playBoards.value.find((pb: { uuid: string }) => pb.uuid === currentBoardUuid).board_type"
              @keydown.esc.stop="closeDrawer"
              @keydown.stop
          ...
          <v-list-item>
            <v-text-field
              label="マス数（横）"
              type="number"
              :model-value="store.playBoards.value.find((pb: { uuid: string, width: string }) => pb.uuid === currentBoardUuid).width"
              @keydown.esc.stop="closeDrawer"
              @keydown.stop
              ref="gridColumnInput"
            />
          ...
          <v-list-item>
            <v-text-field
              label="マス数（縦）"
              type="number"
              :model-value="store.playBoards.value.find((pb: { uuid: string, height: string }) => pb.uuid === currentBoardUuid).height"
              @keydown.esc.stop="closeDrawer"
              @keydown.stop
              ref="gridRowInput"
            />
          ...
          <v-list-item>
            背景色（ペイン）
            <v-color-picker
              v-model="store.playBoards.value.find((pb: { uuid: string, screen_color: string }) => pb.uuid === currentBoardUuid).screen_color"
            />
          </v-list-item>
          <v-list-item>
            背景色（ボード）
            <v-color-picker
              v-model="store.playBoards.value.find((pb: { uuid: string, bg_color: string }) => pb.uuid === currentBoardUuid).bg_color"
            />
          </v-list-item>
          <v-list-item>
            罫線（ボード）
            <v-color-picker
              v-model="store.playBoards.value.find((pb: { uuid: string, border_color: string }) => pb.uuid === currentBoardUuid).border_color"
            />
          </v-list-item>
        </v-defaults-provider>
      </v-list>
