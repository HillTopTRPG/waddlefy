<template>
  <table class="ninpou-table bg-white">
    ...
    <tr v-for="(arts, idx) in list" :key="idx">
      <v-menu :close-on-content-click="false" :z-index="10000000">
        <template v-slot:activator="{props}">
          <td class="name text-decoration-underline" v-bind="props">
            <v-icon :icon="getArtsIcon(arts)" style="width: 1.5rem;" /> {{ arts.name }}
          </td>
        </template>
        <v-container class="arts-detail px-2 pt-2 pb-1">
          <v-defaults-provider :defaults="{
            ...
            <v-row class="mb-1">
              <v-col>
                <p class="overflow-auto pa-1" style="white-space: pre; font-size: 0.7em;">{{ arts.effect || '効果記載なし' }}</p>
              </v-col>
            </v-row>
            <v-row class="mb-1">
              <v-col>
      ...
      <td class="target" @click="emits('click-skill', arts.targetSkill)">{{ arts.targetSkill }}</td>
